...
<div class="page-header">
	...
	<ol class="breadcrumb">
		...
		<li><a href="admin/dashboard/edit/product/">Edit</a></li>
	</ol>
		...
		<div class="col-md-12">
			<div class="panel panel-default">
				...
				<div class="panel-body">
					<form:form action="admin/dashboard/edit/product/"
						...
						<div class="form-group">
							...
							<form:select path="brand.id" cssClass="form-control input-edit">
								<c:forEach var="b" items="${brands}">
									<c:choose>
										<c:when test="${prod.brand.id == b.id}">
						...
						<div class="form-group">
							...
							<form:select path="category.id" cssClass="form-control input-edit">
								<c:forEach var="cate" items="${cates}">
									<c:choose>
										<c:when test="${prod.category.id == cate.id}">
											<option selected value="${cate.id}">${cate.name} - ${cate.nameVN}</option>
										</c:when>
										<c:otherwise>
						...
						<div class="form-group">
							...
							<form:select path="material.id" cssClass="form-control input-edit">
								<c:forEach var="m" items="${mates}">
									<c:choose>
										<c:when test="${prod.material.id == m.id}">
											<option selected value="${m.id}">${m.material} - ${m.materialVN}</option>
										</c:when>
										<c:otherwise>
						...
						<div class="form-group">
							...
							<div id="list-images" class="row" style="margin: auto;">
								<div id="images">
									<c:forEach var="i" items="${prodImages}">
										<div class="img-container shoe-image col-md-3" style="margin-right: 10px; margin-bottom: 10px;">
											<img class="img-upload" src="assets/upload/products/files/${i.image}" height="60" width="85"> 
						...
						<div class="form-group">
							...
							<div id="list-sizes" class="row" style="margin: auto;">
								<c:forEach var="pz" items="${prodSizes}">
									<div class="size-container col-md-3" style="margin-right: 10px; margin-bottom: 10px;">
										<select class="shoe-size input-edit">
											...
											<c:forEach var="sz" items="${sizes}">prodSizes
												<c:choose>
													<c:when test="${sz.id == pz.size.id}">
														<option selected value="${sz.id}">${sz.sizeUS} US - ${sz.sizeVN} VN</option>
													</c:when>
													<c:otherwise>
										...
										<input type="number" value="${pz.quantity}" min="0" max="100" class="shoe-quantity input-edit" />
						...
						<div class="form-group">
							...
							<button id="btn-add" type="submit" class="btn btn-success pull-right">Update</button>
	...
	$(function() {	
		...
		btnUpload.onclick = function() {
            ...
            finder.selectActionFunction = function (fileUrl) {
	    		...
	    		$template.find("img.img-upload").attr("data-url",fileUrl);
	    		$("#list-images #images").append($template.html());
            }
            finder.popup();
		};
		...
		$("#kj-clear-size").click(function() {	
			$("#list-sizes .shoe-size").remove();
			$("#list-sizes .shoe-quantity").remove();	
			disableBtn();
			return false;
		});
